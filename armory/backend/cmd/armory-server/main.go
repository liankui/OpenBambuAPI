...
import (
	...
	// This Service
	"github.com/liankui/OpenBambuAPI/armory/backend/modules/armory-server/api"
)
...
func main() {
	...
	err := config.ScanFrom(&conf, "armoryServer", "server")
	if err != nil {
	...
	if len(gitHash) == 0 {
		logs.Infow("starting ArmoryServer, you can use `-ldflags \"-X main.gitHash=xxx\" to generate gitHash")
	} else {
		logs.Infow("starting ArmoryServer", "gitHash", gitHash, "gitBranch", gitBranch, "buildTime", buildTime)
	}
